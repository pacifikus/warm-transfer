...
- **`warmtransfer.bench`** — бенчмарк (тяжёлые зависимости, extra `bench`).
  - `datasets/` — загрузчики (ML-1M/20M, Goodbooks, KION/KION-text, Amazon Toys/-text, MIND/-text)
  - `splitters/` — честный pseudo-cold сплит (анти-утечка)
  - `adapters/` — модели-доноры (ALS, BPR, CatBoost, EASE, Two-Tower)
...
Grouped MP** на матрице **8 загрузчиков датасетов × 5 доноров** (seed=42): перенос скоров обходит
бейзлайн по per-user AUC в **36 из 40** ячеек датасет×донор (90%). 4 промаха — в основном на ML-1M,
где AUC бейзлайна (~0.72) и так высок. Пять доноров охватывают четыре семейства моделей (матричная
факторизация als/bpr, GBDT catboost, линейный item-item ease, нейросетевой two_tower) — это **ось
разнообразия** для проверки, как держится перенос на разных типах доноров, а не конкуренты для
ранжирования. Наивные методы (knn/attention/debiased/embedding_avg) проигрывают бейзлайну — тянут
популярность соседей. **Оговорка:** это один сид; прицельные многосидовые прогоны на пограничных
ячейках ещё предстоят. Подробности и таблицы — `docs/results/full_matrix.md`.

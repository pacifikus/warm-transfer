...
Grouped MP** на матрице **8 загрузчиков датасетов × 5 доноров** (seed=42):

- по **per-user AUC** — перенос скоров обходит бейзлайн в **36 из 40** ячеек датасет×донор (90%);
- 4 промаха — в основном на ML-1M, где AUC бейзлайна (~0.72) и так высок;
- пять доноров охватывают четыре семейства моделей (матричная факторизация als/bpr, GBDT catboost,
  линейный item-item ease, нейросетевой two_tower) — это **ось разнообразия** для проверки, как
  держится перенос на разных типах доноров, а не конкуренты для ранжирования.
...
ячейках ещё предстоят. Подробности и таблицы — в разделе [Результаты](results/full_matrix.md).
